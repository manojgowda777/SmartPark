                        ...
                        <div className="space-y-8">
                            <div>
                                ...
                                <select 
                                    ...
                                    className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium cursor-pointer hover:bg-white"
                                >
                ...
                <div className="flex-1">
                    <div className="mb-8 flex gap-3">
                        <div className="relative flex-1 group">
                            ...
                            <input 
                                ...
                                className="w-full pl-12 pr-4 py-4 bg-white/80 backdrop-blur-md border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 focus:outline-none transition-all shadow-sm font-medium text-lg hover:bg-white" 
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-8">
                        ...
                        <div className="flex bg-white rounded-xl shadow-sm border border-slate-200 p-1 w-full sm:w-auto overflow-x-auto">
                            <button onClick={() => setViewMode('both')} className={`flex-1 sm:flex-none justify-center px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${viewMode === 'both' ? 'bg-slate-100 text-slate-900 shadow-inner' : 'text-slate-500 hover:text-slate-700'}`}>{t.split}</button>
                            <button onClick={() => setViewMode('map')} className={`flex-1 sm:flex-none justify-center px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${viewMode === 'map' ? 'bg-slate-100 text-slate-900 shadow-inner' : 'text-slate-500 hover:text-slate-700'}`}><MapIcon size={16}/> {t.map}</button>
                            <button onClick={() => setViewMode('list')} className={`flex-1 sm:flex-none justify-center px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${viewMode === 'list' ? 'bg-slate-100 text-slate-900 shadow-inner' : 'text-slate-500 hover:text-slate-700'}`}><List size={16}/> {t.list}</button>
                        </div>
                    </div>
                                    ...
                                    <Marker key={loc.id} position={[loc.latitude, loc.longitude]} icon={customIcon}>
                                        <Popup className="rounded-xl overflow-hidden">
                                            <div className="w-48">
                                                ...
                                                <h3 className="font-bold text-slate-900 leading-tight mb-1">{loc.name}</h3>
                    ...
                    {(viewMode === 'both' || viewMode === 'list') && (
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            {[1, 2, 3, 4].map(i => (
                                <div key={i} className="bg-white p-5 rounded-3xl border border-slate-100 animate-pulse flex flex-col xl:flex-row gap-5 shadow-sm">
                        ...
                        <div className="bg-white/80 backdrop-blur-md p-16 rounded-3xl border border-slate-100 text-center shadow-xl shadow-slate-200/50">
                            <div className="w-24 h-24 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-6">
                                <Search size={40} className="text-slate-400" />
                            </div>
                            <h3 className="text-2xl font-black text-slate-900 mb-3">No parking spots found</h3>
                            <p className="text-slate-500 text-lg">Try changing your city or search filter to find available locations.</p>
                        </div>
                    ) : (
                        <div className="grid md:grid-cols-2 gap-6">
                            {filteredLocations.map(location => (
                                <div key={location.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/40 hover:shadow-2xl hover:shadow-blue-500/20 hover:-translate-y-2 transition-all duration-300 flex flex-col xl:flex-row gap-6 group">
                                    ...
                                    <div className="flex-1 flex flex-col justify-between">
                                        <div>
                                            <div className="flex justify-between items-start gap-2 mb-2">
                                                <h3 className="font-black text-xl text-slate-900 leading-tight group-hover:text-blue-600 transition-colors">{location.name}</h3>
                                        ...
                                        <div className="mt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-slate-100">
                                            <div>
                                                ...
                                                <span className="font-black text-2xl text-slate-900">₹40<span className="text-slate-500 font-medium text-sm">/hr</span></span>
                                            </div>
                                            <Link to={`/parking/${location.id}`} className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-6 py-3 rounded-xl text-sm font-bold transition-all hover:shadow-[0_8px_15px_rgba(79,70,229,0.3)] hover:-translate-y-0.5 text-center whitespace-nowrap">
                                                Book Spot
                                            </Link>
                                        </div>
                                    </div>
