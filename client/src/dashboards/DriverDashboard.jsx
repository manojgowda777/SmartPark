    ...
    return (
        <div className="bg-slate-50 min-h-[calc(100vh-4rem)] p-4 sm:p-8">
            <div className="max-w-7xl mx-auto">
                <h1 className="text-3xl font-bold text-[#0B1533] mb-2">Good morning, {user?.name.split(' ')[0]} ðŸ‘‹</h1>
                <p className="text-slate-600 mb-8">Manage your parking reservations.</p>

                {activeBookings.length > 0 && (
                    <div className="mb-10">
                        <h2 className="text-xl font-bold text-[#0B1533] mb-4 flex items-center gap-2">Active Bookings</h2>
                                ...
                                <div key={booking.id} className="bg-white p-6 rounded-2xl shadow-sm border border-blue-200 relative overflow-hidden flex flex-col justify-between">
                                    <div>
                                        ...
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                <h3 className="text-lg font-bold text-[#0B1533]">{booking.parking_name}</h3>
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-100">
                                            <div>
                                                ...
                                                <p className="text-sm font-medium text-[#0B1533] flex items-center gap-1"><Calendar size={14}/> {new Date(booking.booking_date).toLocaleDateString()}</p>
                                                <p className="text-sm font-medium text-[#0B1533] flex items-center gap-1"><Clock size={14}/> {booking.start_time.slice(0,5)} ({booking.duration}h)</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Amount</p>
                                                <p className="text-lg font-bold text-[#0B1533]">â‚¹{booking.amount}</p>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div>
                    <h2 className="text-xl font-bold text-[#0B1533] mb-4">Booking History</h2>
                    {pastBookings.length === 0 && activeBookings.length === 0 ? (
                        <div className="bg-white p-10 rounded-2xl border border-slate-200 text-center">
                            ...
                            <h3 className="text-lg font-bold text-[#0B1533] mb-2">No bookings yet</h3>
                                ...
                                <tbody>
                                    {bookings.map(booking => (
                                        <tr key={booking.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="p-4 text-sm font-medium text-[#0B1533]">â‚¹{booking.amount}</td>
                                            <td className="p-4">
